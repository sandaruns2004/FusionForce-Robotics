# Quadruped Spider Robot — Project Roadmap

**Assumptions:** 4 legs ("spider/quadruped" configuration), 3 servos/leg (coxa–femur–tibia, 3DOF) = 12 servos total, STM32 board handles low-level servo/PWM and sensor timing, Raspberry Pi handles high-level control, camera/vision, and Wi-Fi/remote control.
...
```
[Camera] --> [Raspberry Pi] <--UART/I2C/SPI--> [STM32] --PWM--> [12x Servos]
                  |                                 |
...
| Component | Qty | Notes |
|---|---|---|
| Micro servos (e.g. MG996R / SG90 depending on load) | 12 | 3 per leg x 4 legs |
| PCA9685 16-channel PWM driver (if STM32 lacks enough PWM pins) | 1 | Offloads PWM generation (16 channels > 12 servos) |
...
- Decide final leg geometry (coxa/femur/tibia link lengths) and gait type (trot or creep gait is standard starting point for 4-leg walkers).
- Choose servo torque based on estimated per-leg load (weight ÷ 2 legs in stance × safety factor).
...
- Assemble one leg first as a prototype; verify range of motion and mechanical clearance before mass-producing 4 legs.
...
- Set up PWM generation for 12 channels (via onboard timers and/or PCA9685 over I2C).
...
### Phase 4 — Gait Engine
- Implement trot gait (diagonal legs move together) or creep gait as the first walking pattern.
...
- **Power sag**: 12 servos moving at once can brown out a Pi if grounds/power aren't properly isolated.
...
*Adjust leg count, servo count, and phase order to match your actual design once finalized — this roadmap assumes a 4-leg/3-servo-per-leg quadruped.*
